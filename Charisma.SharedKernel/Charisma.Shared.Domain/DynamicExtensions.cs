...
namespace Charisma.SharedKernel.Domain
{
    ...
    class PrivateReflectionDynamicObject : DynamicObject
    {
        ...
        {
            if (binder.Type == typeof(Task))
                result = (Task) RealObject;
            else
        ...
        private static object InvokeMemberOnType(Type type, object target, string name, object[] args)
        {
            try
            {
                ...
                //MethodInfo method = type.GetRuntimeMethods().FirstOrDefault(m =>
                //    m.Name == name && 
                //    m.GetParameters()
                //    .Select(p => p.ParameterType).SequenceEqual(args.Select(a => a.GetType())));

                var methods = type.GetRuntimeMethods().Where(m => m.Name == name);
                foreach (var method in methods)
                {
                    ...
                    {
                        var p = parameters[i];
                        var corespondingParam = args[i];

                        if (!p.ParameterType.GetTypeInfo().IsAssignableFrom(corespondingParam.GetType()))
                        {
                            foundMatch = false;
                            break;
                        }
